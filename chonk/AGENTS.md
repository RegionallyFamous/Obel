...
9. **No raw WooCommerce frontend CSS bleeds through.** Any WC block whose default markup is styled by WC's `assets/css/woocommerce.css` (the rounded "folder" product tabs being the canonical example) must be re-styled in `theme.json` using project tokens, with WC's defaults explicitly nullified (`content: none`, `display: none`, `box-shadow: none`, `border-radius: 0`, `border: 0`, etc. as needed). **The override MUST live in top-level `styles.css`, not `styles.blocks.<x>.css`**: WP wraps every block-scoped css string in `:root :where(<block-selector>) { ... }`. `:where()` has specificity zero, so the entire block.css ends up at `(0,0,1)` and WC's `(0,4,3)` always wins. Top-level `styles.css` is emitted verbatim, so write the WC selectors directly (`.woocommerce div.product .woocommerce-tabs ul.tabs li` etc.) and win the cascade by load order (theme after plugin). When you build a new variant, port the WC override block verbatim — never just copy the spacing/typography. The check script (`check_wc_overrides_styled`) enforces this: it requires the WC selectors to appear in top-level `styles.css` with at least one kill declaration, and **fails hard** if it finds a leftover `styles.blocks.<x>.css` field for a known WC surface (currently the single-product tabs).
