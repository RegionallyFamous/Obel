...
Eleven generic patterns in `patterns/`: `hero-image`, `hero-text`, `featured-products`, `value-props`, `faq-accordion`, `testimonials`, `brand-story`, `newsletter`, `cta-banner`, `footer-columns`, `category-tiles`. Six pattern categories registered: `selvedge`, `woo-commerce`, `featured`, `call-to-action`, `testimonials`, `footer`.
...
- Long-form docs (Architecture, Project Structure, Design Tokens, Recipes, Anti-Patterns, Block Reference, Templates, WooCommerce Integration, Style Variations, Working with LLMs, Tooling, Contributing, FAQ) live in the [project wiki](https://github.com/RegionallyFamous/Selvedge/wiki).
