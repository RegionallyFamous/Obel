...
add_action(
	...
	static function (): void {
		$categories = array(
			'foundry'          => array(
				'label'       => __( 'Foundry', 'foundry' ),
				'description' => __( 'Generic starter patterns. Delete or replace per project.', 'foundry' ),
			),
			'woo-commerce'  => array(
				'label'       => __( 'Shop', 'foundry' ),
				'description' => __( 'Patterns for product listings, collections, and shop sections.', 'foundry' ),
			),
			'featured'      => array(
				'label'       => __( 'Hero', 'foundry' ),
				'description' => __( 'Full-width hero and banner patterns.', 'foundry' ),
			),
			'call-to-action' => array(
				'label'       => __( 'Call to action', 'foundry' ),
				'description' => __( 'Conversion-focused banners and newsletter signups.', 'foundry' ),
			),
			'testimonials'  => array(
				'label'       => __( 'Testimonials', 'foundry' ),
				'description' => __( 'Social proof and customer quote patterns.', 'foundry' ),
			),
			'footer'        => array(
				'label'       => __( 'Footer', 'foundry' ),
				'description' => __( 'Footer layout patterns.', 'foundry' ),
			),
		);
...
add_filter(
	...
	static function (): string {
		return '<span class="onsale">' . esc_html__( 'On sale', 'foundry' ) . '</span>';
	}
);
...
add_filter(
	...
	static function ( array $args ): array {
		$args['prev_text'] = '&laquo; prev folio';
		$args['next_text'] = 'next folio &raquo;';
		return $args;
	}
);
...
add_filter(
	...
	static function ( $block_content ) {
		...
		$label = sprintf(
			...
			esc_html( _n( '%d remedy on the shelf', '%d remedies on the shelf', $total, 'foundry' ) ),
			$total
		);
...
add_filter(
	...
	static function ( array $options ): array {
		if ( isset( $options['menu_order'] ) ) {
			$options['menu_order'] = __( 'House order', 'foundry' );
		}
		if ( isset( $options['popularity'] ) ) {
			$options['popularity'] = __( 'Most compounded', 'foundry' );
		}
		if ( isset( $options['rating'] ) ) {
			$options['rating'] = __( 'Highest marked', 'foundry' );
		}
		if ( isset( $options['date'] ) ) {
			$options['date'] = __( 'Freshly compounded', 'foundry' );
		}
		if ( isset( $options['price'] ) ) {
			$options['price'] = __( 'Price, modest to dear', 'foundry' );
		}
		if ( isset( $options['price-desc'] ) ) {
			$options['price-desc'] = __( 'Price, dear to modest', 'foundry' );
		}
		return $options;
	}
);
...
add_filter(
	...
	static function ( $translation, $text, $domain ) {
		...
		// WC default => Foundry voice. Per-theme overrides ship in each
		// theme's functions.php so divergence is visible to the gate
		// (see check_wc_microcopy_distinct_across_themes).
		static $map = array(
			'Estimated total'                                                               => 'Sum due',
			'Proceed to Checkout'                                                           => 'To the counting house',
			'Proceed to checkout'                                                           => 'To the counting house',
			'Lost your password?'                                                           => 'Mislaid your key?',
			'Username or email address'                                                     => 'Apothecary email',
			'Username or Email Address'                                                     => 'Apothecary email',
			'+ Add apartment, suite, etc.'                                                  => 'Additional direction line',
			'You are currently checking out as a guest.'                                    => 'Passing through the counting house? An apothecary account auto-fills the card.',
			'Showing the single result'                                                     => 'Showing I. remedy',
			'Default sorting'                                                               => 'House order',
			'No products were found matching your selection.'                               => 'No remedies match that search, sir.',
			'No products in the cart.'                                                      => 'The ledger is bare.',
			'Your cart is currently empty!'                                                 => 'The ledger is bare.',
			'Your cart is currently empty.'                                                 => 'The ledger is bare.',
			'Return to shop'                                                                => 'Back to the shop floor',
			'Return To Shop'                                                                => 'Back to the shop floor',
			'Have a coupon?'                                                                => 'Voucher in hand?',
			'Update cart'                                                                   => 'Revise the ledger',
			'Place order'                                                                   => 'Dispatch the parcel',
			'Apply coupon'                                                                  => 'Redeem',
			'Coupon code'                                                                   => 'Voucher',
			'Order details'                                                                 => 'Parcel',
			'Order summary'                                                                 => 'Ledger summary',
			'Cart subtotal'                                                                 => 'Running sum',
			'Add to cart'                                                                   => 'Into the parcel',
			'Customer details'                                                              => 'Your particulars',
			'Save my name, email, and website in this browser for the next time I comment.' => 'Note my hand for the next visit.',
			'Be the first to review'                                                        => 'Be the first to commit your hand',
			'Your review'                                                                   => 'Your hand',
			'Your rating'                                                                   => 'Your mark',
			'Submit'                                                                        => 'Inscribe',
			'Description'                                                                   => 'The preparation',
			'Reviews'                                                                       => 'Hands written',
			'Additional information'                                                        => 'The particulars',
			'View cart'                                                                     => 'Open the ledger',
			'View Cart'                                                                     => 'Open the ledger',
			'Choose an option'                                                              => 'Indicate preference',
			'Clear'                                                                         => 'Strike out',
			'Login'                                                                         => 'Enter the apothecary',
			'Log in'                                                                        => 'Enter the apothecary',
			'Log out'                                                                       => 'Close the ledger',
			'Register'                                                                      => 'Open an apothecary account',
			'Remember me'                                                                   => 'Remember the key',
			'My account'                                                                    => 'Apothecary',
			'My Account'                                                                    => 'Apothecary',
			'Order received'                                                                => 'Parcel received, with thanks',
			'Thank you. Your order has been received.'                                      => 'With thanks — your parcel is entered in the ledger.',
			'You may also like&hellip;'                                                     => 'You may also compound',
			'You may also like…'                                                            => 'You may also compound',
			'Related products'                                                              => 'Compounded with these',
		);
...
add_filter(
	...
	static function (): string {
		return __( 'Dispatch the parcel', 'foundry' );
	}
);
	...
	static function ( $field, $key, $args, $value ) {
		if ( false !== strpos( (string) $field, '<abbr class="required"' ) ) {
			$field = preg_replace(
				...
				'<span class="wo-required-mark" aria-hidden="true">&dagger;</span>',
				(string) $field
			);
		}
		return $field;
...
add_action(
	...
	static function (): void {
		...
		add_action( 'woocommerce_account_dashboard', 'foundry_render_account_dashboard' );
	},
	20
);

if ( ! function_exists( 'foundry_render_account_dashboard' ) ) {
	/**
	 ...
	 */
	function foundry_render_account_dashboard(): void {
		...
		if ( '' === $name ) {
			$name = __( 'there', 'foundry' );
		}

		$cards = array(
			array(
				'eyebrow' => __( 'Receipts', 'foundry' ),
				'title'   => __( 'Recent orders', 'foundry' ),
				'lede'    => __( 'Track shipments and reorder favourites in one tap.', 'foundry' ),
				'cta'     => __( 'View orders', 'foundry' ),
				'href'    => wc_get_endpoint_url( 'orders', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Shipping', 'foundry' ),
				'title'   => __( 'Saved addresses', 'foundry' ),
				'lede'    => __( 'Update your billing and delivery details for faster checkout.', 'foundry' ),
				'cta'     => __( 'Edit addresses', 'foundry' ),
				'href'    => wc_get_endpoint_url( 'edit-address', '', wc_get_page_permalink( 'myaccount' ) ),
			),
			array(
				'eyebrow' => __( 'Profile', 'foundry' ),
				'title'   => __( 'Account details', 'foundry' ),
				'lede'    => __( 'Edit your name, email, and password.', 'foundry' ),
				'cta'     => __( 'Edit profile', 'foundry' ),
...
<div class="wo-account-dashboard">
	<header class="wo-account-greeting">
		<p class="wo-account-greeting__eyebrow"><?php esc_html_e( 'Welcome back', 'foundry' ); ?></p>
		<h2 class="wo-account-greeting__title"><?php
			/* translators: %s: customer's first name. */
			echo esc_html( sprintf( __( 'Hello, %s.', 'foundry' ), $name ) );
		?></h2>
		<p class="wo-account-greeting__lede"><?php esc_html_e( 'A quiet record of every order, address, and detail you trust us with — pick up where you left off.', 'foundry' ); ?></p>
	</header>
...
<aside class="wo-account-intro">
	<p class="wo-account-intro__eyebrow"><?php esc_html_e( 'Account', 'foundry' ); ?></p>
	<h2 class="wo-account-intro__title"><?php esc_html_e( 'Welcome back to Foundry.', 'foundry' ); ?></h2>
	<p class="wo-account-intro__lede"><?php esc_html_e( 'Sign in to view your order history, track shipments, and check out faster next time.', 'foundry' ); ?></p>
	<ul class="wo-account-intro__perks">
		<li><?php esc_html_e( 'One-click reorders', 'foundry' ); ?></li>
		<li><?php esc_html_e( 'Saved addresses + payment', 'foundry' ); ?></li>
		<li><?php esc_html_e( 'Wishlist + early access drops', 'foundry' ); ?></li>
	</ul>
</aside>
		<?php
	},
...
<p class="wo-account-help"><?php
	printf(
		...
		esc_html__( 'Trouble signing in? %s.', 'foundry' ),
		'<a href="' . esc_url( '/contact/' ) . '">' . esc_html__( 'Get in touch', 'foundry' ) . '</a>'
	);
?></p>
		<?php
	},
...
//
// Per-theme: each theme owns its own `foundry_swatches_color_map()` so
...
if ( ! function_exists( 'foundry_swatches_render_group' ) ) {
	/**
	 ...
	 */
	function foundry_swatches_render_group( string $default_html, array $args ): string {
		...
		$colors  = foundry_swatches_color_map();
		$buttons = '';
...
add_filter(
	...
	static function ( $html, $args ) {
		...
		return foundry_swatches_render_group( (string) $html, (array) $args );
	},
	20,
	2
);
...
<div class="wo-empty wo-empty--cart">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Cart', 'foundry' ); ?></p>
	<h1 class="wo-empty__title"><?php esc_html_e( 'Your cart is rather empty.', 'foundry' ); ?></h1>
	<p class="wo-empty__lede"><?php esc_html_e( 'Browse the shop, or pick up a thread you left in the journal.', 'foundry' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'Continue browsing', 'foundry' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'foundry' ); ?></a>
	</p>
</div>
		<?php
	},
...
<div class="wo-empty wo-empty--shop">
	<p class="wo-empty__eyebrow"><?php esc_html_e( 'Shop', 'foundry' ); ?></p>
	<h2 class="wo-empty__title"><?php esc_html_e( 'Nothing matches that filter just yet.', 'foundry' ); ?></h2>
	<p class="wo-empty__lede"><?php esc_html_e( 'Try a different category, or see everything currently in stock.', 'foundry' ); ?></p>
	<p class="wo-empty__ctas">
		<a class="wo-empty__cta wo-empty__cta--primary" href="<?php echo esc_url( '/shop/' ); ?>"><?php esc_html_e( 'See everything', 'foundry' ); ?></a>
		<a class="wo-empty__cta wo-empty__cta--secondary" href="<?php echo esc_url( '/journal/' ); ?>"><?php esc_html_e( 'Visit the journal', 'foundry' ); ?></a>
	</p>
</div>
				<?php
...
add_action(
	...
	static function (): void {
		...
		if ( $has_term ) {
			...
			if ( '' === $title ) {
				$title = __( 'Shop', 'foundry' );
			}
			$eyebrow = __( 'Shop', 'foundry' );
		}
...
add_action(
	...
	static function (): void {
		...
		$label = esc_js( __( 'We accept', 'foundry' ) );
		?>
<script>
(function(){
