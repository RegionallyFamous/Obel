...
6. **No raw WooCommerce frontend CSS bleeds through.** WC ships opinionated frontend CSS that survives a block theme (rounded "folder" product tabs, hard-coded notice colours, `.button` overrides, etc.). Any WC surface whose default markup is styled by `plugins/woocommerce/assets/css/woocommerce.css` MUST be re-styled in `theme.json` using project tokens, with the WC defaults explicitly nullified (`content: none`, `display: none`, `box-shadow: none`, `border-radius: 0`, `border: 0`, etc.). Known surfaces: `.woocommerce-tabs ul.tabs` (single-product tabs), `woocommerce/store-notices`, `woocommerce/cart`/`woocommerce/checkout` form rows, the legacy `.button` class. **The override MUST live in top-level `styles.css`, not `styles.blocks.<x>.css`**: WP processes block-scoped css through `WP_Theme_JSON::process_blocks_custom_css()`, which wraps every rule in `:root :where(<block-selector>) { ... }`. `:where()` has specificity zero, so the entire block.css string ends up at `(0,0,1)` — WC's `(0,4,3)` always wins. Top-level `styles.css` is emitted verbatim, so we can write the WC selectors with their natural specificity (`.woocommerce div.product .woocommerce-tabs ul.tabs li` etc.) and win the cascade by load order (theme after plugin). When you add a new theme variant, port the full WC override block — never just the spacing/typography. `bin/check.py` enforces this for the known surfaces and **fails hard** if it finds a leftover block-scoped css field for them.
